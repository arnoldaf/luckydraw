...
class Spreadsheet_Excel_Reader {
	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		...
		echo $sql="INSERT INTO imei_data_import set imei1 ='".$this->val($row,'1',$sheet)."',imei2='".$this->val($row,'2',$sheet)."',sn='".$this->val($row,'3',$sheet)."',sale_date='".$this->val($row,'4',$sheet)."-".$this->val($row,'5',$sheet)."-".$this->val($row,'6',$sheet)."',model='".$this->val($row,'7',$sheet)."',party_name='".$this->val($row,'8',$sheet)."',sale_location='".$this->val($row,'9',$sheet)."',status='I'";
mysql_query($sql)or die(mysql_error());
	...
	function dump2($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		...
		//$val = $this->val($row,$col,$sheet);
		/////////////Checking blank row in excel file////////////////
		if(($this->val($row,'1',$sheet))=="" ){
		}
		else{
	//////////////////////Checking duplicate entry////////////////////////////		
			$get_imei="select imei1 from imei_data_import where imei1='".$this->val($row,'1',$sheet)."' or imei2='".$this->val($row,'1',$sheet)."' or imei1='".$this->val($row,'2',$sheet)."' or imei2='".$this->val($row,'2',$sheet)."'";
			$imei_exist=mysql_query($get_imei)or die(mysql_error());
			if(mysql_num_rows($imei_exist)>0){
				
				}
			else{
		 $sql="INSERT INTO imei_data_import set imei1 ='".$this->val($row,'1',$sheet)."',imei2='".$this->val($row,'2',$sheet)."',sn='".$this->val($row,'3',$sheet)."',sale_date='".$this->val($row,'4',$sheet)."-".$this->val($row,'5',$sheet)."-".$this->val($row,'6',$sheet)."',model='".$this->val($row,'7',$sheet)."',party_name='".$this->val($row,'8',$sheet)."',sale_location='".$this->val($row,'9',$sheet)."',status='I'";
mysql_query($sql)or die(mysql_error());
			}
		}
				}
				header("Location:excel_upload_imei_import.php?msg=sucess");
exit;
	}
	// --------------
	// END PUBLIC API
